...
class PostService:
    ...
    async def _invalidate_posts_cache(self, user_id: int) -> None:
        redis = await get_redis()
        async for key in redis.scan_iter(match=f"user:{user_id}:posts:*"):
            await redis.delete(key)
        async for key in redis.scan_iter(match="all:posts:*"):
            await redis.delete(key)

    async def create_post(self, user_id: int, title: str, text: str) -> Post:
        post = await self.repository.create(user_id, title, text)
        await self._invalidate_posts_cache(user_id)
        return post
    ...
    async def update_post(self, post_id: int, title: Optional[str], text: Optional[str]) -> Optional[Post]:
        ...
        if post is not None:
            await self._invalidate_posts_cache(post.user_id)
        return post
    ...
    async def get_all_posts(self, limit: int = 10, offset: int = 0) -> List[PostResponse]:
        redis = await get_redis()
        cache_key = f"all:posts:{limit}:{offset}"
